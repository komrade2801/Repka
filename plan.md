...
- [ ] Полировка UI/UX: скелетоны, тоасты (уведомлений) для обработки ошибок.
  - *Частично:* текстовый лоадер задач, спиннер и inline-ошибки в чате / импорте — без библиотеки тостов и скелетонов.
- [x] Подготовка демо-данных: `backend/seed.py` (сидинг ~130 задач с приоритетами и зависимостями в SQLite).
  - [ ] Отдельный тестовый `.xlsx` для демо импорта (ещё нет в репозитории).
...
| Этап | Статус |
| --- | --- |
| 1–4 | **готово** (ядро MVP + усиленный Гантт / priority) |
| 5 | **в работе** — seed готов; экспорт Excel и UX-полировка (тосты/скелетоны) — нет |
| 6 | **не начат** (кроме черновика README) |
